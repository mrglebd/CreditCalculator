...
namespace CreditCalculator.Services
{
    public class DailyScheduleCalculator : IRepaymentScheduleCalculator
    {
        ...
        public ResultViewModel Calculate(CreditInputModel input)
        {
            ...
            decimal dailyRate = input.DailyInterestRate!.Value;

            if (stepDays <= 0 || totalDays <= 0)
                throw new ArgumentException("Invalid loan duration or step");

            int numberOfPayments = (int)Math.Ceiling((decimal)totalDays / stepDays);
            decimal periodRate = (dailyRate / 100m) * stepDays;

            decimal pow = FinanceHelper.Pow(1 + periodRate, numberOfPayments);
            decimal payment = loanAmount * periodRate * pow / (pow - 1);
            ...
            {
                decimal interest = remainingDebt * periodRate;
